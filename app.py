...
class AcademicReportGenerator:
    ...
    def generate(self, results, analysis_type):
        ...
        if analysis_type == 'descriptive':
            ...
            return self._format_chi_square(results)
    ...
    def _format_chi_square(self, r):
        """تقرير اختبار مربع كاي الأكاديمي"""
        if 'error' in r:
            return f"❌ خطأ: {r['error']}"
        
        report = "═"*55 + "\n"
        report += "   اختبار مربع كاي للاستقلالية - Chi-Square Test\n"
        report += "═"*55 + "\n\n"
        
        report += "📊 أولاً: جدول التكرارات المتقاطعة\n"
        report += "─"*55 + "\n\n"
        
        report += f"المتغير الأول: {r['المتغير_1']}\n"
        report += f"المتغير الثاني: {r['المتغير_2']}\n\n"
        
        # بناء الجدول
        table = r['جدول_التكرارات']
        if len(table) > 1:
            # استخراج أسماء الأعمدة
            header_cols = [k for k in table[0].keys() if k not in ['الفئة', 'المجموع']]
            ...
            # صفوف البيانات
            for row in table:
                report += f"│ {str(row['الفئة']):<12} "
                for col in header_cols:
                    val = row.get(col, 0)
                    report += f"│ {str(val):>12} "
                report += f"│ {str(row.get('المجموع', 0)):>8} │\n"
            
            report += "└" + "─"*60 + "┘\n\n"
        
        report += "📈 ثانياً: نتائج اختبار مربع كاي\n"
        report += "─"*55 + "\n\n"
        
        report += f"   • قيمة χ² المحسوبة = {r['chi2']}\n"
        report += f"   • درجات الحرية (df) = {r['df']}\n"
        report += f"   • مستوى الدلالة (p) = {r['p']}\n"
        report += f"   • معامل كرامر V = {r['cramers_v']}\n"
        ...
        if r['دال']:
            report += f"نلاحظ من خلال النتائج أن قيمة χ² المحسوبة بلغت ({r['chi2']})\n"
            report += f"وهي قيمة دالة إحصائياً عند مستوى دلالة (α = {r['مستوى_الدلالة']})\n"
            report += f"حيث كانت قيمة p = {r['p']}، وهي أقل من 0.05.\n\n"
            
            report += "وبناءً على ذلك، نرفض الفرضية الصفرية ونقبل الفرضية البديلة،\n"
            report += f"مما يعني وجود علاقة ذات دلالة إحصائية بين {r['المتغير_1']}\n"
            report += f"و{r['المتغير_2']}.\n\n"
            
            report += f"كما أن حجم الأثر (Cramér's V = {r['cramers_v']}) يُصنف على أنه\n"
            report += f"{r['حجم_الأثر']}، مما يشير إلى قوة العلاقة بين المتغيرين.\n"
        else:
            report += f"نلاحظ من خلال النتائج أن قيمة χ² المحسوبة بلغت ({r['chi2']})\n"
            report += f"وهي قيمة غير دالة إحصائياً، حيث كانت قيمة p = {r['p']}\n"
            report += "وهي أكبر من 0.05.\n\n"
            ...
            report += "وبناءً على ذلك، نقبل الفرضية الصفرية، مما يعني عدم وجود\n"
            report += f"علاقة ذات دلالة إحصائية بين {r['المتغير_1']} و{r['المتغير_2']}.\n"
        ...
        if r['دال']:
            report += "▪ في فصل النتائج:\n"
            report += f'\"أظهرت نتائج اختبار χ² وجود علاقة دالة إحصائياً بين\n'
            report += f'{r["المتغير_1"]} و{r["المتغير_2"]},\n'
            report += f'χ²({r["df"]}) = {r["chi2"]}, p = {r["p"]},\n'
            report += f'Cramér\'s V = {r["cramers_v"]}\"\n\n'
        else:
            report += "▪ في فصل النتائج:\n"
            report += f'\"لم تظهر نتائج اختبار χ² علاقة دالة إحصائياً بين\n'
            report += f'{r["المتغير_1"]} و{r["المتغير_2"]},\n'
            report += f'χ²({r["df"]}) = {r["chi2"]}, p = {r["p"]}\"\n\n'
        ...
        report += "📚 المراجع المقترحة:\n"
        report += "─"*55 + "\n"
        report += "• McHugh, M. L. (2013). The Chi-square test of\n"
        report += "  independence. Biochemia Medica, 23(2), 143-149.\n\n"
        report += "• Field, A. (2013). Discovering Statistics Using IBM\n"
        report += "  SPSS Statistics (4th ed.). SAGE Publications.\n"
        report += "═"*55 + "\n"
    ...
    def _format_cronbach(self, r):
        ...
        report += f"بلغت قيمة معامل ألفا كرونباخ ({r['alpha']})، وهي قيمة تُصنف\n"
        report += f"على أنها {r['التصنيف']} وفقاً للمعايير المتعارف عليها.\n\n"
        
        if r['alpha'] >= 0.70:
            report += "وهذا يشير إلى أن المقياس يتمتع بثبات داخلي جيد، مما يعني\n"
            report += "أن البنود متسقة فيما بينها وتقيس نفس البُنية النظرية.\n\n"
        else:
            report += "وهذا يشير إلى ضرورة مراجعة بعض البنود أو إضافة بنود جديدة\n"
            report += "لتحسين الثبات الداخلي للمقياس.\n\n"
        
        report += "من خلال عمود 'α إذا حُذف'، يمكن ملاحظة البنود التي قد\n"
        report += "يؤدي حذفها إلى تحسين أو خفض الثبات الكلي للمقياس.\n"
        
        report += "\n" + "─"*55 + "\n"
        report += "📝 كيفية الكتابة في المذكرة:\n"
        report += "─"*55 + "\n\n"
        
        report += "▪ في فصل الإجراءات المنهجية:\n"
        report += f'\"تم التحقق من ثبات المقياس باستخدام معامل ألفا كرونباخ،\n'
        report += f'حيث بلغت قيمته (α = {r["alpha"]})، وهي قيمة {r["التصنيف"]}\n'
        report += 'تشير إلى ثبات داخلي مناسب للمقياس.\"\n\n'
        
        report += "▪ في جدول خصائص أدوات الدراسة:\n"
        report += "يمكن إدراج جدول إحصاءات البنود أعلاه مباشرة.\n"
        
        report += "\n" + "─"*55 + "\n"
        report += "📚 المراجع المقترحة:\n"
        report += "─"*55 + "\n"
        report += "• Cronbach, L. J. (1951). Coefficient alpha and the\n"
        report += "  internal structure of tests. Psychometrika, 16(3),\n"
        report += "  297-334.\n\n"
        report += "• Tavakol, M., & Dennick, R. (2011). Making sense of\n"
        report += "  Cronbach's alpha. International Journal of Medical\n"
        report += "  Education, 2, 53-55.\n"
        report += "═"*55 + "\n"
